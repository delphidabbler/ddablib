...
<ol class="spaced">
  <li>
    ...
    a form from the component palette.
  </li>
  <li>
  ...
  <li>
    ...
    Demonstrates how to use <var>TPJUserWdwState</var> and load and save data
    in the <var>OnReadData</var> and <var>OnSaveData</var> events.
  </li>
  <li>
    <strong><code>MDIDemo.dpr</code></strong><br />
    Demonstrates how to use the components with MDI applications. This demo uses
    <var>TPJWdwState</var> components with the <var>woFitWorkArea</var> option
    to keep MDI child forms within the main form's client area.
  </li>
</ol>
...
<p>
  On Delphi XE2 and later the demos can be compiled for 64 bit Windows. Proceed
  as follows:
</p>

<ul>
  <li>
    Ensure the package where the Window State Components have been installed has
    been compiled for a Windows 64 bit target.
  </li>
  <li>
    Add a Windows 64 target to each demo project.
  </li>
  <li>
    Add the directory containing the 64 bit package containing the components to
    the each demo project's search path for the 64 bit target.
  </li>
</ul>
...
<p>
  ...
  displays a modal dialogue box. Both the main window and the dialogue contain a
  <var>TPJRegWdwState</var> component. The component in the main window is
  configured to persist window size, state and position. The dialogue's
  component only persists window position. Window positions are not adjusted to
  remain within the work space.
</p>

<p>
  Both forms demonstrate the use of the <var>OnReadWdwState</var> event. The
  main form also shows how to use the <var>OnGetRegDataEx</var>,
...
<p>
  The program uses the <var>TPJWdwState</var> component's <var>IniRootDir</var>,
  ...
  file named <code>StandAloneDemo.ini</code> in the
  <code>%AppData%\DelphiDabbler\Lib\PJWdwState</code> directory. It also shows
  how to use the <var>IniFilePath</var> method to get the full file spec. of
  the ini file.
</p>
...
<p>
  ...
  <var>OnReadData</var> and <var>OnSaveData</var> events. <var>OnSaveData</var>
  is used to write the content of the <var>Data</var> record passed to the event
  handler to a file as binary data. <var>OnReadData</var> reads any stored data
...
<p>
  This simple demo simply shows how to use the window state components with MDI
  applications. There are two forms in the project &ndash; a main form and a
  child form. Neither form has any code, but both forms have a
  <var>TPJWdwState</var> component set up to write to different sections of the
  same ini file. The main form has a toolbar, a menu bar and a status bar. These
  controls have been added to show that the window state components can make
  allowances for furniture in the main when keeping child forms within the
  available work area.
</p>
...
<p>
  ...
  makes. You may also want to try removing the component from the main form and
  once again seeing what effect this has on the result.
</p>
...
<div class="endnotes">
  ...
  <div class="comments">
    If you have any comments on the components or this demo code or if you have
    found a bug, please let me know. Bugs should be reported via the <a
