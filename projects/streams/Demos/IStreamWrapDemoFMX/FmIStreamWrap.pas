{
 * Main form for IStreamWrapFMX Stream Library Demo Program.
 *
 * $Rev$
 * $Date$
 *
...
type
  TIStreamWrapForm = class(TForm)
    ...
    sbStatus: TStatusBar;
    lblStatusBarSection0: TLabel;
    lblStatusBarSection1: TLabel;
...
begin
  PopupPos := ClientToScreen(
    PointF(btnFSStats.Position.X, btnFSStats.Position.Y + btnFSStats.Height)
  );
...
begin
  ...
  try
    ...
    OleCheck(fIMemStm.Seek(0, STREAM_SEEK_SET, StreamStart));
    ...
    UpdateStatusBar(
      1, 'Bytes read from Memory Stream = ' + IntToStr(BytesRead)
    );
  finally
    // Free the buffer
    FreeMem(Buf);
...
begin
  ...
  FileStm := TPJIStreamWrapper.Create(
    TFileStream.Create(cTestFileName, fmCreate), True
  );
  ...
  case MI.Tag of
    0:
      ...
      Stm := TPJIStreamWrapper.Create(
        TFileStream.Create(cTestFileName, fmOpenRead), True
      );
    1:
    begin
      ...
      Stm := TPJHandleIStreamWrapper.Create(
        THandleStream.Create(FileH), True
      );
    end;
    2:
...
begin
  if PanelID = 0 then
    lblStatusBarSection0.Text := Msg
  else if PanelID = 1 then
    lblStatusBarSection1.Text := Msg;
end;
